...
@pytest.mark.asyncio
async def test_initialize_gets_coroutine_function():
...
@pytest.mark.asyncio
async def test_initialize_gets_coroutine():
    worker = AsyncMock()
    assert worker.call_args_list == []
    job = CustomJob(name='foo', label='Foo', worker=worker)
    await job._task
    assert worker.call_args_list == [call()]
...
async def test_wait(mocker):
    job = CustomJob(name='foo', label='Foo', worker=asyncio.sleep(1))
