...
class SceneCheckJob(JobBase):
    ...
    async def _find_release_name(self):
        ...
        if is_scene_release is types.SceneCheckResult.false:
            ...
            self._finalize(types.SceneCheckResult.false, exceptions=())
        else:
    ...
    async def _verify_release(self, release_name):
        ...
        self._finalize(is_scene_release, exceptions)
    ...
    def user_decided(self, is_scene_release):
        """
        Must be called by the UI when the user decided whether this is a scene
        release or not
        """
